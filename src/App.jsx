...
const INSTRUCTIONS_URL = 'https://santhosh72744.github.io/instructions/'; // ← Replace this URL
...
function ConfirmDialog({message,onConfirm,onCancel}){
  return(
    <div className="fade-in" style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',
      display:'flex',alignItems:'center',justifyContent:'center',zIndex:2100,padding:'24px'}}>
      <div className="scale-in" style={{background:`linear-gradient(145deg,${C.navyMid},#0a1628)`,
...
function LoginScreen({onLogin}){
  ...
  const submit=()=>{
    ...
    setTimeout(()=>{
      if(user.trim()===CREDS.user&&pass===CREDS.pass){
        sessionStorage.setItem('cmr_auth','1');
        onLogin();
  ...
  return(
    <div style={{minHeight:'100vh',background:`radial-gradient(ellipse at 50% 0%,#12284a 0%,${C.bg} 65%)`,
      ...
      {[['8%','18%','60px','15deg','0'],['82%','12%','40px','-10deg','0.8s'],
        ...
        <div key={i} style={{position:'absolute',left:l,top:t,fontSize:fs,opacity:0.04,
          transform:`rotate(${rot})`,pointerEvents:'none',
          animation:`crossGlow 4s ease-in-out infinite`,animationDelay:del}}>✝</div>
      ))}

      <div className="scale-in" style={{width:'100%',maxWidth:'380px'}}>
        {/* Logo */}
        <div style={{textAlign:'center',marginBottom:'36px'}}>
          <div style={{width:'84px',height:'84px',borderRadius:'50%',margin:'0 auto 16px',
            ...
            fontSize:'40px',boxShadow:'0 0 50px rgba(212,168,67,0.18)'}}>✝</div>
          <h1 style={{fontFamily:'Cinzel,Georgia,serif',fontSize:'26px',fontWeight:'900',
            color:C.white,letterSpacing:'2px',marginBottom:'6px'}}>AMOR DEI MINISTRIES</h1>
          <p style={{color:C.gray,fontSize:'12px',letterSpacing:'2px'}}>CHURCH ASSIGNMENT MANAGER</p>
        </div>

        {/* Card */}
        <div style={{background:'linear-gradient(145deg,rgba(15,30,60,0.9),rgba(8,16,36,0.97))',
          ...
          <p style={{color:C.gray,fontSize:'13px',marginBottom:'24px',textAlign:'center',letterSpacing:'0.5px'}}>
            Sign in to continue
          </p>

          {/* Username */}
          <div style={{marginBottom:'16px'}}>
            <label style={{display:'block',color:C.gray,fontSize:'10px',letterSpacing:'2px',
              textTransform:'uppercase',marginBottom:'8px',fontWeight:'700'}}>Username</label>
            <div style={{position:'relative'}}>
              <span style={{position:'absolute',left:'14px',top:'50%',transform:'translateY(-50%)',fontSize:'16px'}}>👤</span>
              <input value={user} onChange={e=>setUser(e.target.value)} onKeyDown={e=>e.key==='Enter'&&submit()}
                placeholder="admin"
                style={{width:'100%',background:'rgba(255,255,255,0.05)',
                  border:`1px solid ${err?C.redBdr:C.glassBdr}`,borderRadius:'12px',
                  padding:'14px 14px 14px 44px',color:C.white,fontSize:'15px',fontFamily:'Lato,sans-serif'}}
              />
            </div>
          </div>
          ...
          <div style={{marginBottom:'24px'}}>
            <label style={{display:'block',color:C.gray,fontSize:'10px',letterSpacing:'2px',
              textTransform:'uppercase',marginBottom:'8px',fontWeight:'700'}}>Password</label>
            <div style={{position:'relative'}}>
              <span style={{position:'absolute',left:'14px',top:'50%',transform:'translateY(-50%)',fontSize:'16px'}}>🔒</span>
              <input value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==='Enter'&&submit()}
                ...
                style={{width:'100%',background:'rgba(255,255,255,0.05)',
                  border:`1px solid ${err?C.redBdr:C.glassBdr}`,borderRadius:'12px',
                  padding:'14px 44px 14px 44px',color:C.white,fontSize:'15px',fontFamily:'Lato,sans-serif'}}
              />
              <button onClick={()=>setShow(s=>!s)} style={{position:'absolute',right:'14px',
                top:'50%',transform:'translateY(-50%)',background:'none',border:'none',cursor:'pointer',fontSize:'16px'}}>
                {show?'🙈':'👁️'}
              </button>
            </div>
          </div>
          ...
          {err&&(
            <div style={{background:C.redDim,border:`1px solid ${C.redBdr}`,borderRadius:'10px',
              padding:'10px 14px',marginBottom:'16px',color:C.red,fontSize:'13px',textAlign:'center'}}>
              ⚠️ {err}
            </div>
          )}
        ...
        <p style={{textAlign:'center',color:'rgba(143,163,188,0.3)',fontSize:'11px',marginTop:'20px',letterSpacing:'0.5px'}}>
...
function MainApp({onLogout}){
  ...
  const[minForm,  setMinForm]  =useState({name:'',icon:'✝️'});
  ...
  const handleGenerate=async()=>{
    ...
    try{
      ...
      setRoster(nr);setTab('roster');showToast('Roster generated! ✨');
  ...
  const handleConfirm=async()=>{
    setActionLoad(true);
    try{const u=await api.confirmRoster(roster._id);setRoster(u);showToast('Roster confirmed ✅');}
    catch(e){showToast(e.message,'error');}
    finally{setActionLoad(false);}
  };
    ...
    setConfirm({
      ...
      onConfirm:async()=>{
        ...
        try{
          ...
          if(isCurrent)setRoster(null);
          else setHistory(h=>h.filter(r=>r._id!==id));
  ...
  const handleAddMember=async(ministryId,memberId)=>{
    setActionLoad(true);
    try{const u=await api.addMember(roster._id,ministryId,memberId);setRoster(u);setModal(null);showToast('Member added ✅');}
    catch(e){showToast(e.message,'error');}
    finally{setActionLoad(false);}
  };
  ...
  const handleRemoveMember=async(ministryId,memberId)=>{
    setActionLoad(true);
    try{const u=await api.removeMember(roster._id,ministryId,memberId);setRoster(u);showToast('Member removed 🔄');}
    catch(e){showToast(e.message,'error');}
    finally{setActionLoad(false);}
  };

  // Returns interested members NOT already assigned to this ministry
  const getAddCandidates=(ministryId)=>{
    const assignment=(roster?.assignments||[]).find(a=>(a.ministryId?._id||a.ministryId)?.toString()===ministryId);
  ...
  const deleteMember=(id)=>{
    setConfirm({message:'This member and their interests will be removed.',onConfirm:async()=>{
      setConfirm(null);setActionLoad(true);
  ...
  const saveMinistry=async()=>{
    ...
    setActionLoad(true);
    try{
      if(minEditing){const u=await api.updateMinistry(minEditing,name,minForm.icon);setMinistries(p=>p.map(m=>m._id===minEditing?u:m));setMinEditing(null);showToast('Ministry updated');}
      else{
        const icon=minForm.icon!=='✝️'?minForm.icon:getAutoIcon(name);
        const c=await api.createMinistry(name,icon);setMinistries(p=>[...p,c]);showToast('Ministry added ✅');
      }
      setMinForm({name:'',icon:'✝️'});
    }catch(e){showToast(e.message,'error');}
    finally{setActionLoad(false);}
  };

  const deleteMinistry=(id)=>{
    setConfirm({message:'This ministry and related interests will be removed.',onConfirm:async()=>{
      setConfirm(null);setActionLoad(true);
  ...
  const generatePoster=()=>{
    if(!roster)return;
    const W=820,rowH=76,asgn=roster.assignments||[];
    const H=320+asgn.length*rowH+100;
    ...
    const draw=()=>{
      ...
      ctx.fillStyle=C.gold;ctx.font='bold 56px Georgia,serif';ctx.textAlign='center';ctx.fillText('✝',W/2,96);
      ctx.fillStyle=C.goldLt;ctx.font='bold 36px Georgia,serif';ctx.fillText(churchName.toUpperCase(),W/2,156);
      ctx.fillStyle='rgba(240,244,255,0.9)';ctx.font='20px Georgia,serif';ctx.fillText('SUNDAY MINISTRIES',W/2,192);
      ctx.fillStyle=C.gold;ctx.font='14px Georgia,serif';ctx.fillText(roster.date,W/2,218);
      ctx.strokeStyle=C.gold;ctx.lineWidth=1.5;ctx.setLineDash([6,5]);
      ctx.beginPath();ctx.moveTo(60,238);ctx.lineTo(W-60,238);ctx.stroke();ctx.setLineDash([]);
      let y=292;
      asgn.forEach((a,i)=>{
        const min=a.ministryId,mem=a.memberId;
        ctx.fillStyle=i%2===0?'rgba(212,168,67,0.07)':'rgba(255,255,255,0.02)';
        rr(ctx,54,y-32,W-108,rowH-8,10);ctx.fill();
        ctx.fillStyle=C.goldLt;ctx.font='bold 18px Georgia,serif';ctx.textAlign='left';
        const mLabel=`${min?.icon||'•'} ${min?.name||''}`;
        ctx.fillText(mLabel,74,y);
        const mW=ctx.measureText(mLabel).width;
        ctx.strokeStyle='rgba(212,168,67,0.15)';ctx.lineWidth=1;ctx.setLineDash([3,6]);
        ctx.beginPath();ctx.moveTo(74+mW+18,y-5);ctx.lineTo(W-148,y-5);ctx.stroke();ctx.setLineDash([]);
        ctx.fillStyle=mem?'#f0f4ff':'#f87171';ctx.font='18px Georgia,serif';ctx.textAlign='right';
        const names=(a.memberIds||[]).map(m=>m?.name).filter(Boolean).join(', ')||'— Unassigned —';
        ctx.fillStyle=names==='— Unassigned —'?'#f87171':'#f0f4ff';
        ctx.fillText(names,W-74,y);
        y+=rowH;
      });
      ctx.strokeStyle=C.gold;ctx.lineWidth=1.5;ctx.setLineDash([6,5]);
      ctx.beginPath();ctx.moveTo(60,y+10);ctx.lineTo(W-60,y+10);ctx.stroke();ctx.setLineDash([]);
      ctx.fillStyle=C.gold;ctx.font='14px Georgia,serif';ctx.textAlign='center';ctx.fillText('May God bless your service! 🙏',W/2,H-56);
      ctx.fillStyle='rgba(255,255,255,0.2)';ctx.font='11px sans-serif';ctx.fillText('AMOR DEI MINISTRIES',W/2,H-34);
    ...
    <div style={{padding:'20px'}}>
      ...
      <div className="fade-up" style={{textAlign:'center',marginBottom:'24px',
        ...
        <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',
          fontSize:'140px',opacity:0.04,pointerEvents:'none',animation:'crossGlow 5s ease-in-out infinite'}}>✝</div>
      ...
      {roster&&(
        <button className="btn-press fade-up s3" onClick={()=>setTab('roster')} style={{
            ...
            <GlassCard key={h._id} className={`fade-up s${Math.min(idx+1,5)}`}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <div>
                  ...
                  <div style={{color:C.gray,fontSize:'12px',marginTop:'3px'}}>{h.assignments?.length} assignments</div>
                </div>
                <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                  <Badge color={h.status==='confirmed'?C.green:C.gold} bg={h.status==='confirmed'?C.greenDim:C.goldDim}>
                    {h.status==='confirmed'?'✅ Done':'📝 Draft'}
                  </Badge>
  ...
  const renderMembers=()=>(
    <div style={{padding:'20px'}}>
      ...
      <GlassCard className="fade-up s1">
        <div style={{display:'flex',gap:'10px'}}>
          <input value={mForm} onChange={e=>setMForm(e.target.value)} onKeyDown={e=>e.key==='Enter'&&saveMember()}
            ...
            style={{flex:1,background:'rgba(255,255,255,0.05)',border:`1px solid ${C.glassBdr}`,
              borderRadius:'12px',padding:'13px 16px',color:C.white,fontSize:'15px',fontFamily:'Lato,sans-serif'}}
          />
      ...
      {members.length===0&&(
        <div style={{textAlign:'center',color:C.gray,padding:'40px 20px',fontSize:'14px'}}>
          <div style={{fontSize:'48px',marginBottom:'12px'}}>👥</div>No members yet.
        </div>
      )}
      {members.map((m,idx)=>{
      ...
      <GlassCard className="fade-up s1">
        ...
        <div style={{display:'flex',gap:'10px'}}>
          <input value={minForm.name} onChange={e=>setMinForm(f=>({...f,name:e.target.value}))} onKeyDown={e=>e.key==='Enter'&&saveMinistry()}
            placeholder={minEditing?'Edit name…':'Ministry name (icon auto-detected)…'}
            style={{flex:1,background:'rgba(255,255,255,0.05)',border:`1px solid ${C.glassBdr}`,
              borderRadius:'12px',padding:'13px 16px',color:C.white,fontSize:'15px',fontFamily:'Lato,sans-serif'}}
          />
          <GoldBtn onClick={saveMinistry} disabled={actionLoad}>{minEditing?'✓':'+'}</GoldBtn>
          {minEditing&&<DangerBtn onClick={()=>{setMinEditing(null);setMinForm({name:'',icon:'✝️'});}}>✕</DangerBtn>}
        </div>
      </GlassCard>
      {ministries.length===0&&(
        <div style={{textAlign:'center',color:C.gray,padding:'40px 20px',fontSize:'14px'}}>
          <div style={{fontSize:'48px',marginBottom:'12px'}}>⛪</div>No ministries yet.
        </div>
      )}
          ...
          <GlassCard key={m._id} className={`fade-up s${Math.min(idx+2,5)}`} style={{borderColor:minEditing===m._id?C.goldBdr:C.glassBdr}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div style={{display:'flex',alignItems:'center',gap:'16px'}}>
                <div style={{width:'52px',height:'52px',borderRadius:'14px',background:C.goldDim,
                  border:`1px solid rgba(212,168,67,0.2)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'26px'}}>
                  {m.icon}
                </div>
                <div>
                  <div style={{color:C.white,fontSize:'16px',fontWeight:'700',fontFamily:'Cinzel,Georgia,serif'}}>{m.name}</div>
                  <div style={{color:C.gray,fontSize:'12px',marginTop:'3px'}}>{count} interested member{count!==1?'s':''}</div>
                </div>
              </div>
              <div style={{display:'flex',gap:'6px'}}>
                <EditBtn onClick={()=>{setMinEditing(m._id);setMinForm({name:m.name,icon:m.icon});}}>✏️</EditBtn>
  ...
  const renderRoster=()=>{
    ...
    const asgn=roster.assignments||[];
    const unassigned=asgn.filter(a=>!a.memberIds||a.memberIds.length===0).length;

    return(
      <div style={{padding:'20px'}}>
        {/* Header card */}
        <div className="fade-up" style={{background:'linear-gradient(145deg,rgba(15,30,60,0.7),rgba(10,20,40,0.5))',
          border:`1px solid ${C.glassBdr}`,borderRadius:'18px',padding:'18px',marginBottom:'16px'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
            <div>
              <div style={{color:C.white,fontSize:'20px',fontFamily:'Cinzel,Georgia,serif',fontWeight:'700'}}>Sunday Roster</div>
              <div style={{color:C.gray,fontSize:'13px',marginTop:'4px'}}>{roster.date}</div>
            </div>
            <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:'8px'}}>
              <Badge color={roster.status==='confirmed'?C.green:C.gold} bg={roster.status==='confirmed'?C.greenDim:C.goldDim}>
                {roster.status==='confirmed'?'✅ Confirmed':'📝 Draft'}
              </Badge>
              <button onClick={()=>handleDeleteRoster(roster._id,true)} style={{
                background:C.redDim,border:`1px solid ${C.redBdr}`,borderRadius:'8px',
        ...
        {asgn.map((a,idx)=>{
          ...
          return(
            <GlassCard key={minId} className={`fade-up s${Math.min(idx+1,5)}`}
              style={{borderColor:hasMembers?C.glassBdr:C.redBdr}}>
              {/* Ministry label row */}
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}}>
                <div style={{color:C.goldLt,fontSize:'11px',letterSpacing:'1px',
                  textTransform:'uppercase',fontWeight:'700'}}>
                  {min?.icon} {min?.name}
                </div>
                <button onClick={()=>setModal({type:'add',ministryId:minId})}
                  className="btn-press" style={{padding:'6px 12px',background:C.goldDim,
              ...
              {!hasMembers&&(
                <div style={{color:C.red,fontSize:'14px',fontWeight:'600',fontFamily:'Cinzel,Georgia,serif'}}>
                  ⚠️ No one assigned
                </div>
              )}
              <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                {memberIds.map((mem,mi)=>{
                  ...
                  return(
                    <div key={memId||mi} style={{display:'flex',justifyContent:'space-between',
                      alignItems:'center',background:'rgba(255,255,255,0.04)',
                      borderRadius:'10px',padding:'10px 14px',
                      border:`1px solid ${C.glassBdr}`}}>
                      <div style={{color:C.white,fontSize:'15px',fontWeight:'700',fontFamily:'Cinzel,Georgia,serif'}}>
                        👤 {mem?.name||'Unknown'}
                      </div>
                      <button onClick={()=>handleRemoveMember(minId,memId)}
                        className="btn-press" style={{background:C.redDim,border:`1px solid ${C.redBdr}`,
                          borderRadius:'8px',color:C.red,cursor:'pointer',padding:'4px 9px',fontSize:'12px',fontWeight:'700'}}>
                        ✕
                      </button>
                    </div>
                  );
  ...
  const renderModal=()=>{
    ...
    return(
      <div className="fade-in" onClick={()=>setModal(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',
        display:'flex',alignItems:'flex-end',justifyContent:'center',zIndex:1000}}>
        <div onClick={e=>e.stopPropagation()} style={{background:`linear-gradient(145deg,${C.navyMid},#0a1628)`,
          ...
          <div style={{width:'40px',height:'4px',background:'rgba(255,255,255,0.12)',
            borderRadius:'2px',margin:'0 auto 22px'}} onClick={()=>setModal(null)}/>
          <div style={{color:C.goldLt,fontSize:'11px',letterSpacing:'1.5px',textTransform:'uppercase',fontWeight:'700'}}>
            {min?.icon} {min?.name}
          </div>
          <div style={{color:C.white,fontSize:'20px',fontWeight:'700',fontFamily:'Cinzel,Georgia,serif',marginTop:'4px',marginBottom:'6px'}}>
            Add Person
          </div>
          <div style={{color:C.gray,fontSize:'13px',marginBottom:'20px'}}>
            Members interested in this ministry and not yet assigned here
          </div>
          {candidates.length===0
            ?<GlassCard style={{textAlign:'center',borderColor:C.redBdr,marginBottom:'12px'}}>
              <div style={{fontSize:'28px',marginBottom:'8px'}}>😔</div>
              <div style={{color:C.red,fontSize:'14px',fontWeight:'700'}}>No one available to add</div>
              <div style={{color:C.gray,fontSize:'12px',marginTop:'4px'}}>
                All interested members are already added, or go to Interests tab to assign interest.
              </div>
            </GlassCard>
            :candidates.map(s=>(
  ...
  const renderPreview=()=>{
    ...
    return(
      <div className="fade-in" style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.96)',
        ...
        <div className="scale-in" style={{width:'100%',maxWidth:'420px',textAlign:'center'}}>
          <div style={{color:C.white,fontSize:'18px',fontWeight:'700',marginBottom:'16px',fontFamily:'Cinzel,Georgia,serif'}}>🖼️ Roster Poster Ready</div>
          <img src={posterImg} alt='Poster' style={{maxWidth:'100%',maxHeight:'52vh',borderRadius:'16px',
          ...
          <div style={{display:'flex',gap:'12px'}}>
            <button onClick={()=>{const a=document.createElement('a');a.href=posterImg;a.download=`ministry-roster-${new Date().toISOString().split('T')[0]}.png`;a.click();}}
              className="btn-press" style={{flex:1,padding:'16px',background:`linear-gradient(135deg,${C.gold},${C.goldLt})`,
                border:'none',borderRadius:'14px',color:'#07100a',fontSize:'15px',fontWeight:'700',cursor:'pointer',fontFamily:'Cinzel,Georgia,serif'}}>
              ⬇️ Download
            </button>
            <button onClick={()=>setShowPreview(false)} className="btn-press" style={{flex:1,padding:'16px',
              background:'rgba(255,255,255,0.07)',border:`1px solid ${C.glassBdr}`,borderRadius:'14px',
              color:C.white,cursor:'pointer',fontFamily:'Lato,sans-serif'}}>Close</button>
          </div>
        </div>
      </div>
    );
  ...
  const renderToast=()=>toast&&(
    <div className="scale-in" style={{position:'fixed',bottom:'84px',left:'50%',transform:'translateX(-50%)',
  ...
  return(
    <div style={{minHeight:'100vh',background:C.bg,color:C.white,maxWidth:'480px',margin:'0 auto',position:'relative'}}>
      {/* Header */}
      <div style={{background:'rgba(4,9,26,0.97)',borderBottom:`1px solid ${C.glassBdr}`,
        ...
        {actionLoad?<Spinner size={20}/>:(
          <button onClick={onLogout} style={{background:C.redDim,border:`1px solid ${C.redBdr}`,
      ...
      <nav style={{position:'fixed',bottom:0,left:'50%',transform:'translateX(-50%)',
        ...
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} className="btn-press" style={{
            ...
            <span style={{fontSize:'10px',color:tab===t.id?C.gold:C.gray,fontFamily:'Lato,sans-serif',
              fontWeight:tab===t.id?'700':'400',letterSpacing:'0.3px'}}>{t.label}</span>
          </button>
        ))}
      </nav>
